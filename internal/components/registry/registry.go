...
type Registry struct {
	lock     sync.RWMutex
	models   []components.Coremodel
	modelIdx map[registryKey]components.Coremodel
}

// NewRegistry returns a new Registry with models.
func NewRegistry(models ...components.Coremodel) (*Registry, error) {
	size := len(models)

	r := &Registry{
		models:   make([]components.Coremodel, 0, size),
		modelIdx: make(map[registryKey]components.Coremodel, size),
	}
...
func (r *Registry) Register(models ...components.Coremodel) error {
	return r.addModels(models)
}

// List returns all coremodels registered in this Registry.
func (r *Registry) List() []components.Coremodel {
	r.lock.RLock()
...
func (r *Registry) addModels(models []components.Coremodel) error {
	r.lock.Lock()
	defer r.lock.Unlock()
