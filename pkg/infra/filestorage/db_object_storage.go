...
type objectInSQL struct {
	...
	Properties           map[string]string `xorm:"properties"`
	MimeType             string            `xorm:"mime_type"`
}

var (
	objectColsNoContents = []string{"path", "path_hash", "parent_folder_path_hash", "etag", "cache_control", "content_disposition", "updated", "created", "size", "mime_type"}
	allObjectCols        = append([]string{"contents"}, fileColsNoContents...)
)

type dbObjectStorage struct {
	db  db.DB
	log log.Logger
}

func NewDbObjectStorage(log log.Logger, db db.DB, filter PathFilter, rootFolder string) FileStorage {
	return newWrapper(log, &dbObjectStorage{
		log: log,
		db:  db,
	}, filter, rootFolder)
}

func (s dbObjectStorage) Get(ctx context.Context, path string, options *GetFileOptions) (*File, bool, error) {
	var result *File
...
func (s dbObjectStorage) Delete(ctx context.Context, filePath string) error {
...
func (s dbObjectStorage) Upsert(ctx context.Context, cmd *UpsertFileCommand) error {
	now := time.Now()
...
//nolint:gocyclo
func (s dbObjectStorage) List(ctx context.Context, folderPath string, paging *Paging, options *ListOptions) (*ListResponse, error) {
	...
	err := s.db.WithDbSession(ctx, func(sess *sqlstore.DBSession) error {
		...
		propertiesByPathHash := make(map[string]map[string]string, 0) //s.getProperties(sess, hashes)
		if err != nil {
			return err
		}
...
func (s dbObjectStorage) CreateFolder(ctx context.Context, path string) error {
	now := time.Now()
...
func (s dbObjectStorage) DeleteFolder(ctx context.Context, folderPath string, options *DeleteFolderOptions) error {
	lowerFolderPath := strings.ToLower(folderPath)
	if lowerFolderPath == "" || lowerFolderPath == Delimiter {
		lowerFolderPath = Delimiter
...
func (s dbObjectStorage) close() error {
	return nil
}
