...
func loadGetMetricDataOutputs(path string) ([]*cloudwatch.GetMetricDataOutput, error) {
...
func TestCloudWatchResponseParser(t *testing.T) {
	t.Run("when aggregating response", func(t *testing.T) {
		getMetricDataOutputs, err := loadGetMetricDataOutputs("./test-data/multiple-outputs.json")
	...
	// 	}
	// 	frames, partialData, err := buildDataFrames(mdrs, labels, query)
	// 	require.NoError(t, err)
