...
func Test_PluginsInstallAndUninstall(t *testing.T) {
	...
	for _, tc := range tcs {
		...
		t.Run(testName("Install", tc), func(t *testing.T) {
			...
			webtest.RequestWithSignedInUser(req, &user.SignedInUser{UserId: 1, OrgId: 1, OrgRole: user.RoleEditor, IsGrafanaAdmin: true})
		...
		t.Run(testName("Uninstall", tc), func(t *testing.T) {
			...
			webtest.RequestWithSignedInUser(req, &user.SignedInUser{UserId: 1, OrgId: 1, OrgRole: user.RoleViewer, IsGrafanaAdmin: true})
			resp, err := srv.SendJSON(req)
			require.NoError(t, err)
