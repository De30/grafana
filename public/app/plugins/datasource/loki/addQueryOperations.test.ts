...
describe('addOperations()', () => {
  it('should add selector to simple query', () => {
    expect(() => {
      addOperations('', { addSelector: { key: 'foo', operator: '', value: '' } });
    }).toThrow();
    expect(addOperations('{}', { addSelector: { key: 'bar', operator: '=', value: 'baz' } })).toBe('{bar="baz"}');
    expect(addOperations('{x="yy"}', { addSelector: { key: 'bar', operator: '=', value: 'baz' } })).toBe(
      '{x="yy",bar="baz"}'
    );
  });

  it('should add not equal selector', () => {
    expect(addOperations('{}', { addSelector: { key: 'bar', operator: '!=', value: 'baz' } })).toBe('{bar!="baz"}');
    expect(addOperations('{x="yy"}', { addSelector: { key: 'bar', operator: '!=', value: 'baz' } })).toBe(
      '{x="yy",bar!="baz"}'
    );
  ...
  it('should not modify ranges', () => {
    expect(addOperations('rate({}[1m])', { addSelector: { key: 'foo', operator: '=', value: 'bar' } })).toBe(
      'rate({foo="bar"}[1m])'
    );
  });
  ...
  it('should detect in-order function use', () => {
    expect(
      addOperations('sum by (xx) (rate{}[5m])', { addSelector: { key: 'bar', operator: '=', value: 'baz' } })
    ).toBe('sum by (xx) (rate{bar="baz"}[5m])');
  });

  it('should correctly use +Inf', () => {
    expect(
      addOperations('sum(rate({}[5m])) by (le)', { addSelector: { key: 'le', operator: '=', value: '+Inf' } })
    ).toBe('sum(rate({le="+Inf"}[5m])) by (le)');
  });

  it('should handle selectors with punctuation', () => {
    expect(
      addOperations('{instance="my-host.com:9100"}', { addSelector: { key: 'bar', operator: '=', value: 'baz' } })
    ).toBe('{instance="my-host.com:9100",bar="baz"}');
    expect(addOperations('{list="a,b,c"}', { addSelector: { key: 'bar', operator: '=', value: 'baz' } })).toBe(
      '{list="a,b,c",bar="baz"}'
    );
  });

  it('should work on arithmetical expressions', () => {
    expect(addOperations('{} + {}', { addSelector: { key: 'bar', operator: '=', value: 'baz' } })).toBe(
      '{bar="baz"} + {bar="baz"}'
    );
