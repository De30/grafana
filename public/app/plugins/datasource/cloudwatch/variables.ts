...
export class CloudWatchVariableSupport extends CustomVariableSupport<CloudWatchDatasource, VariableQuery> {
  ...
  async handleLogGroupsQuery({ region, logGroupPrefix }: VariableQuery) {
    const logGroups: string[] = await this.datasource.logsQueryRunner.describeAllLogGroups({
      region,
  ...
  async handleRegionsQuery() {
    const regions = await this.datasource.api.getRegions();
  ...
  async handleNamespacesQuery() {
    const namespaces = await this.datasource.api.getNamespaces();
  ...
  async handleMetricsQuery({ namespace, region }: VariableQuery) {
    const metrics = await this.datasource.api.getMetrics(namespace, region);
  ...
  async handleDimensionKeysQuery({ namespace, region }: VariableQuery) {
    const keys = await this.datasource.api.getDimensionKeys(namespace, region);
  ...
  async handleDimensionValuesQuery({ namespace, region, dimensionKey, metricName, dimensionFilters }: VariableQuery) {
    ...
    const keys = await this.datasource.api.getDimensionValues(
      region,
      namespace,
      metricName,
      dimensionKey,
  ...
  async handleEbsVolumeIdsQuery({ region, instanceID }: VariableQuery) {
    ...
    const ids = await this.datasource.api.getEbsVolumeIds(region, instanceID);
  ...
  async handleEc2InstanceAttributeQuery({ region, attributeName, ec2Filters }: VariableQuery) {
    ...
    const values = await this.datasource.api.getEc2InstanceAttribute(region, attributeName, ec2Filters ?? {});
  ...
  async handleResourceARNsQuery({ region, resourceType, tags }: VariableQuery) {
    ...
    const keys = await this.datasource.api.getResourceARNs(region, resourceType, tags ?? {});
    return keys.map((s: { label: string; value: string }) => ({
      text: s.label,
      value: s.value,
      expandable: true,
