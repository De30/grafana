...
function filterAlertStates(
  rules: CombinedRule[],
  state: PromAlertingRuleState | 'all'
): Array<CombinedRule<AlertingRule>> {
  return rules.filter((rule): rule is CombinedRule<AlertingRule> => {
    if (isAlertingRule(rule.promRule)) {
      return state !== 'all' ? rule.promRule?.state === state : true;
    }
 ...
 * @TODO
 *  RBAC
 *  Make this work with recording rules
 */
export const RuleListGroupView: FC<Props> = ({ namespaces }) => {
  ...
  const GROUPED_BY_STATE = {
    [PromAlertingRuleState.Firing]: filterAlertStates(allAlertRules, PromAlertingRuleState.Firing),
    [PromAlertingRuleState.Pending]: filterAlertStates(allAlertRules, PromAlertingRuleState.Pending),
                          ...
                          <HoverCard
                            ...
                            content={
                              <Card className={styles.cardReset}>
                                <Card.Heading>{group.name}</Card.Heading>
                                <Card.Meta>
                                  <span>
                                    <Icon name="clock-nine" /> Evaluate every {group.interval ?? 'default'}
                                  </span>
                                  <span>
                                    {group.rules.length} {pluralize('alert rule', group.rules.length)}
                                  </span>
                                </Card.Meta>
                                <Card.Figure>
                                  <img
                                ...
                                <Card.Actions>
                                  <Button icon="pen" variant="secondary" size="sm">
                                    Edit
                                  </Button>
                                  <Button icon="trash-alt" variant="destructive" size="sm">
                                    Delete
                                  </Button>
                                </Card.Actions>
                              </Card>
                          ...
                          >
                            <div>
                              <Stack alignItems={'baseline'}>
                                <Icon name="angle-down" /> {group.name}
                              </Stack>
                            </div>
                          </HoverCard>
                        </Stack>
